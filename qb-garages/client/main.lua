...
local function SetCarItemsInfo()
	local items = {}
    local job = QBCore.Functions.GetPlayerData().job.name
	for _, item in pairs(JobVehicles[job].trunk) do
...
local function SpawnVehicleSpawnerVehicle(vehicleModel, location, heading, cb)
    QBCore.Functions.SpawnVehicle(vehicleModel, function(veh)
        ...
        if JobVehicles[job].enabledtrunk then
            SetCarItemsInfo()
            TriggerServerEvent("inventory:server:addTrunkItems", QBCore.Functions.GetPlate(veh), JobVehicles[job].trunk)
        end
...
RegisterNetEvent('lty:client:spawnjobcars', function(data, cb)
    local garageType = data.type
    local vehicleModel = data.vehicleModel
    -- local vehicle = data.vehicle
    local garage = data.garage
    ...
    local location, heading = GetSpawnLocationAndHeading(garage, garageType, parkingSpots, spawnDistance)
    if garage.useVehicleSpawner then
        SpawnVehicleSpawnerVehicle(vehicleModel, location, heading, cb)
    end
end)
